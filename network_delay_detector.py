...
class NetworkDelayDetector(app_manager.RyuApp):
    ...
    def _detector(self):
        ...
        """
        while CONF.weight == 'delay' or CONF.weight == 'all':
    ...
    def echo_reply_handler(self, ev):
        ...
        echo_timestamp = float(ev.msg.data)
        try:
            latency = now_timestamp - echo_timestamp
    ...
    def show_delay_statis(self):
        if setting.TOSHOW and self.awareness is not None:
            self.latency_stats_pretty.clear()
            ...
            for src in self.awareness.graph:
                for dst in self.awareness.graph[src]:
                    ...
                    self.latency_stats_pretty.append({
                        'src': src,
                        'dst': dst,
                        'latency': delay
                    })
                    self.logger.info("%s<-->%s : %.5f" % (src, dst, delay))
                    
    def map_delay_to_score(self, delay):
        return math.exp(-1.5 * delay)